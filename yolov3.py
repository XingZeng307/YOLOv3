...
import torch
'''
config list explain --> transform the original config file into simplified version
"D": Detection Block
"U": Upsample layer
"R": ResNet Block
"Y": Yolo Block
...
there are other things which are used to concatenate feature maps from different layers, such as Upsampling, Route
'''
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            if isinstance(layer, DetectionBlock):
                ...
                results.append(
                    layer(x).reshape(x.shape[0],  x.shape[2], x.shape[3], 3, self.num_classes + 5)
                )
                continue
